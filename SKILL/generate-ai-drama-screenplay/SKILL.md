...
6. For every shot, include `visual_description`; `action` cannot substitute for it. Also include `characters`, `reference_assets`, `shot_type`, `camera_angle`, `lens`, `camera_action`, `emotion`, `keyframe_prompt`, `negative_prompt`, `video_prompt`, `h3_prompt`, and `status: "pending_review"`. Keep Qwen prompts structured Chinese and Flux prompts sentence-like English when both engines are planned.
7. Add continuity states to the shot plan: what is true at the start (`continuity_in`) and at the end (`continuity_out`). State positions, facing direction, hand/prop state, emotional level, lighting, and any door or furniture state. A later shot must inherit the previous shot's end state unless there is an intentional hard cut.
8. Create a continuous prompt chain. For same-angle motion, instruct the video workflow to use the previous clip's tail frame as the next clip's start image. For a new angle, describe the inherited tail state and mark the transition as a hard cut at a pause in action or dialogue. Never pretend a new keyframe is a seamless morph when the camera angle changes.
