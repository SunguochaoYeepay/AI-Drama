...
Create one compact character identity sheet per visual character before scene prompts. This is an identity asset, not a story frame and not an influencer portrait. Require exactly three equally sized views of the same person (front, left three-quarter, and right profile), chest-up or half-body framing, eye-level camera, neutral seamless light-gray background, soft even studio light, and a calm neutral expression. Do not add full-body thumbnails, a fourth view, multiple poses, props, room backgrounds, or extra people. Repeat the locked age, face shape, hairline, hair length, skin texture, body build, wardrobe colors, fabric, seams, and accessories in every view. Generate multiple candidates, approve one, and keep its asset ID in every later shot. For a crowded keyframe, use one approved crop from this sheet if the three-view sheet still overloads the image model. Do not put dialogue, dramatic emotion, or scene action in this reference sheet.
