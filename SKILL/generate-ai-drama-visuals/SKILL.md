...
- Treat `reference_assets` as the references for this generation request, not as the package inventory. Send at most five image references to Qwen/Flux for one keyframe: one location plate, the visible foreground/active characters, and zero or one hero prop. Do not include both a location ID and its location plate, or every recurring prop at once. Keep the full asset inventory in the package and canvas for later shots.
