...
每个有对白的镜头都必须在视频/H3 提示词中明确写出“谁在说话”和“说什么”。静态 Qwen/Flux 生图提示词只标记当前说话者和自然嘴型，不放完整台词，避免图像模型把文字当成画面内容。`dialogue_ids` 只是程序引用，不能代替给视频模型的自然语言指令。每个场景还要先生成并审核场景参考图，以及餐桌、手机、账本、作业本、餐具等连续性关键道具的参考图。
...
当前家庭故事制作包中的 `assets` 已定义客餐厅场景板、木质餐桌、家庭账本、黑色手机、作业本与铅笔、陶瓷碗六类参考资产，状态为 `pending_generation`。道具参考图必须生成“同一物品六视角”的独立参考板，使用浅中性灰背景，不保留房间、餐桌或其他环境；场景参考图单独控制环境。这些是制作计划，不是已经存在的图片资源；当前结构化导入器还不会自动把它们转换成 DramaClaw 的原生场景/道具资源。短期需要在 DramaClaw 的场景和道具页面分别生成，长期需要让导入器完成 `assets` 到原生资源的映射。没有可访问的真实文件地址时，不要填写虚构的 `url`。

视觉资产和关键帧使用不同的提示词层：人物资产是中性背景的四视角定妆板，锁定脸型、发型、年龄、身材和服装；场景资产是无人物、无活动道具的空间板，只锁定门、门洞、墙角、家具和灯光位置；道具资产是中性背景的同一物品六视角参考板。关键帧才负责剧情动作、对白、表情、镜头构图和人物站位。重新生成资产时应先确认人物，再确认场景和道具，最后生成关键帧。
...
同样不要一次关联全部参考图。`reference_assets` 是本次生成请求的输入，不是资产库存；关键帧默认只关联 1 张场景板和当前可见人物，不关联道具图，最多 5 张。人物参考图使用简洁三视图（正面、三分之四、侧面），三个视图等大，不带全身缩略图和多余姿势；拥挤镜头仍不稳定时，再改用其中一张已审核的单人裁图。静态提示词必须按上游顺序写明 `@图片1=场景；@图片2=人物A；@图片3=人物B`，让 DramaClaw 的引用编号和 Qwen 输入保持一致。完整人物、场景和道具卡片仍保留在自由画布中，后续物品特写再单独选择。

### 连续视频链

每个镜头先由剧本 Skill 决定转场，而不是每段都预先生一张关键帧：`tail_frame_continue` 直接把上一段视频尾帧作为下一段 H3 首帧；`tail_frame_reframe` 则在自然停顿处截尾帧，作为 Qwen Edit 的 `@图片1`，配合最多两张人物定妆图生成新机位关键帧。首镜头才直接生成关键帧。尾帧尚未存在时，不填写虚构 URL，后续关键帧节点应处于“等待上一段视频尾帧”状态。
