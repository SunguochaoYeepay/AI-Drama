...
当前家庭故事制作包中的 `assets` 已定义客餐厅场景板、木质餐桌、家庭账本、黑色手机、作业本与铅笔、陶瓷碗六类参考资产，状态为 `pending_generation`。这些是制作计划，不是已经存在的图片资源；当前结构化导入器还不会自动把它们转换成 DramaClaw 的原生场景/道具资源。短期需要在 DramaClaw 的场景和道具页面分别生成，长期需要让导入器完成 `assets` 到原生资源的映射。没有可访问的真实文件地址时，不要填写虚构的 `url`。
